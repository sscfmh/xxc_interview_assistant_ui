...
import {
  createQuestionQcRel,
  deleteQuestionQcRelById,
  pageQueryQuestionQcRel,
  updateQuestionQcRelById,
} from "@/api/questionQcRelApi";
...
export default function QuestionQcRel() {
  ...
  const handlePageQuery = useCallback(async () => {
    try {
      await pageQueryQuestionQcRel(pageQueryReq).then((res) => {
        if (res.success) {
          setPageQueryResult({
...
const Header = () => {
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
      <div>
        <h2 className="text-2xl font-bold">QuestionQcRel</h2>
        <p className="mt-1 text-gray-600 dark:text-gray-300">Manage QuestionQcRel</p>
      </div>
      <Button
        type="primary"
      ...
      >
        <span>Add New QuestionQcRel</span>
      </Button>
    </div>
  );
};
  ...
  return (
    <Space size="middle">
      <Button
        onClick={() => {
          setDetail({
            title: `QuestionQcRel ID = ${record.id}`,
            descMeta,
      ...
      <Popconfirm
        onConfirm={() => {
          deleteQuestionQcRelById(record.id).then((res) => {
            if (res.success) {
              setShouldQuery(true);
            }
          });
  ...
  return (
    ...
    >
      <Form
        ...
        name="questionQcRelAddOrEditModalForm"
        onFinish={(formValues) => {
          const data = {
            ...formValues,
          ...
          if (isUpdate) {
            updateQuestionQcRelById(data).then((res) => {
              if (res.success) {
                setAddOrEditModalShow(false);
                addOrEditModalForm.resetFields();
                setShouldQuery(true);
            ...
            createQuestionQcRel(data).then((res) => {
              if (res.success) {
                setAddOrEditModalShow(false);
                addOrEditModalForm.resetFields();
                setShouldQuery(true);
